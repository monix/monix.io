...
layout: docs
title: Observer
type_api: monix.reactive.Observer
type_source: monix-reactive/shared/src/main/scala/monix/reactive/Observer.scala
description: |
    A listener type that can be called asynchronously with the events of a reactive stream. Used by the Monix Observable.
---
...
Monix provides various `Observer` implementations in the 
[monix.reactive.observers]({{ site.api2x }}#monix.reactive.observers.package){:target="_blank"}
package.
